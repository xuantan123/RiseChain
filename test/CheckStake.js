...
async function main() {
  ...
  try {
    ...
    const [pricePerFullShare, totalShares, totalLockedAmount] = await Promise.all([
        ZetaStake.getPricePerFullShare(),
        ZetaStake.totalShares(),
        ZetaStake.totalLockedAmount()
    ]);

    console.log("✅ Vault Info:");
    console.log("Price Per Full Share:", new BigNumber(pricePerFullShare.toString()).toFixed());
    console.log("Total Shares:", new BigNumber(totalShares.toString()).toFixed());
    console.log("Total Locked Amount:", new BigNumber(totalLockedAmount.toString()).toFixed());
  } catch (err) {
    console.error("❌ Lỗi khi gọi contract:", err);
  }
}
...
main()
  ...
  // npx hardhat run test/CheckStake.js --network ZeroGravityChain